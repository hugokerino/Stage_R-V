...
# -*- coding: utf-8 -*-
"""
Created on Thu May  4 09:51:15 2023
...
start = time.time()
y, yerr = generate_regular_data_H0(params_gr, params_act, N, t)
...
y_ir2 = np.zeros(Ttot)
yerr_ir2 = np.zeros(Ttot)
t_ir2 = np.zeros(Ttot)

indices = np.sort(np.random.choice(N,Ttot,replace=False))
count=0
for i in range(Ttot):
    y_ir2[i], yerr_ir2[i], t_ir2[i]  = y[indices[count]], yerr[indices[count]], t[indices[count]]
    count +=1
